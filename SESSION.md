...
- 机制类的活基本干完了(采集 → trace → 审计 → 强制),卡点全部集中在证据采集这一段。
- 分支:`codex/publish-research`,最新提交 `56364f7 Merge pull request #56`。
- open PR:**0**。#49–#56 全部合并(#55 与 #56 是 stacked,已按 #55 → #56 顺序合入)。
- open issue:**#57**(唯一硬阻塞)、#17、#16、#1。#43、#38、#39、#45、#30 相关工作已落地。
- 评估结论未变:`evaluation: evidence_insufficient` / `efficiency_claim: not_supported`。

## 本轮已完成

### 六题配对 6/6,采集链本身是干净的
12 条 trace 全部 `complete`、`warnings: []`、0 条 partial;12/12 工作区摘要吻合(未观测执行 0、未过闸执行 0);审计器从原始证据重算 7 个字段、不采信采集器自述,6/6 配对通过。140 个策略决策事件,含 5 次 deny、2 次 rewrite 生效 —— L2 改写路径首次拿到配对证据。

六题 `quality_claim_eligible: true` 全过,卡在 `conclusion` 那一层:**是数量不够,不是质量不合格。**

### 阈值核准:场景覆盖不是 1/4,是 1/2
`src/verification-benchmark.mjs:18` 写的是 `MINIMUM_GENERALIZED_SCENARIO_CLASSES = 2` —— 门槛是覆盖 **2 类**。四类(`cli_tool` / `web_frontend` / `service_library` / `research_script`)是**枚举全集,不是要求**。

这条纠正改变了排法:三个已过资格的外部 fixture 正好落在 `service_library`(pino)、`web_frontend`(zustand)、`cli_tool`(yargs),**只要在 pino 或 zustand 上出一道题,这一格立刻满足**。之前看板按 1/4 画,把一件差一步的事画成了差三步。

### 看板与文档已对齐
`docs/progress-2026-08-25.html`:修正场景覆盖为 1/2、分支与提交号、删掉已失效的"还没提交"整节(三个审计脚本已签入,文档按行号引用的证据不再悬空)、按解锁关系重排下一步表。`STATUS.md` 从 08-19 口径更新到 08-25。

## 未完成

- **有效 oracle 失败样本 0/10(#57)。** `vp_flaky_retry_once` 的 flaky 标记在 `src/verification-workspace.mjs:196` 是一次性的,被 Agent 必须的第一次运行消耗掉,oracle 在 post-run 副本里必然 pass、必然报 failed。两臂 oracle 报告逐字节相同,零信息量。30 对能靠时间堆出来,失败样本不行 —— 它要求 oracle 能因为**正确的原因**失败。
- **配对比较数 6/30**,还缺 24 对。
- **场景覆盖 1/2**、**外部 fixture 已出题 0/3**。环境资格过了不等于已观测到该场景下的验证行为 —— 场景覆盖按**题目**算,不按 fixture 算。
- **闸门在虚耗调用。** 闸门用整串正则、判定按段分解,两把尺子不一致:`rg` 命令的搜索模式里出现 `pytest` 就被当成测试命令,逐段分解找不到 runner,fail-closed 拒掉。`vp_unknown_impact_full_fallback` 过闸 31 次白丢 5 次。

### 8-24 那轮六题数字不能读成效率信号
两个污染源都已量化:① codex 在并行批里只要有一个调用被 hook 拒掉,会把同批兄弟调用一起丢掉,所以那 5 个"批准了但从没执行"的调用是 harness 掉的,不是策略省的;② 闸门缺陷本身在虚耗调用。`vp_unknown_impact_full_fallback` 那条 14 → 26 的暴涨,读的是这两个缺陷,不是策略的真实开销。

## 下一步

顺序按"解锁了什么"排,不按工作量。1 和 2 必须在 3 之前(它们决定采集到的数据算不算数),3 必须在 4 之前(它验证流水线)。

1. **拍板 #57 的 oracle 怎么修 —— 唯一需要人拍的设计决定。** `minimum_oracle_failures` 是 `src/evaluation.mjs:9` 的独立门槛项。先修再采,采到的才算数;先采后修,前面采的全废。倾向方案:声明这题不由 oracle 判、只认 trace 证据,同时让审计器标出"结构上判不了的 oracle"。三条路里唯一不破坏"审计独立于采集"这条主纪律的,代价只是新增一类审计标记,而该标记本身有复用价值 —— 以后能自动拦住同类恒为 failed 的判定。
2. **闸门改按段判定(v0.5)后重采。** 必须在扩量之前,否则 30 对里每一对都掺同样噪声,攒完还是不能下结论。
3. **在 `pino` 上出第一道题。** 一道题同时解掉"场景覆盖 1/2"和"样本自指",并验证外部出题这条流水线。
4. **扩到 30 对。** 纯堆量,24 对按六题一轮算四轮,可后台批跑。
