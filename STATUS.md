...
- **外部 fixture 已出题 2/3。** 环境资格过了不等于已观测到该场景下的验证行为 —— 场景覆盖按**题目**算,不按 fixture 算。pino 有 2 题(#59 诊断题、#67 修复题);yargs / zustand 仍 0 题,资格早就过了但没题就不进覆盖。资格实跑:带 `--fixture-repo external-pino=PATH` 时 `fixture_ready: 8/8`;CI 无外部 checkout 时两题记 `skipped` 并在 blockers 里点名 `external_fixture_checkouts_not_supplied`,不冒充已观测。
- **出题连着撬出两处「声明了但没实现」。** #71:`agentEditAssessment` 不读 oracle 声明的 `production_edits` / `test_edits`,从 `task.mode` 反推,三个取值里两个不可达。紧接着资格验证也按 `behavior_class` 反推退出码期望,使 `mode: end_to_end` 从 schema 落地起就不可用 —— 修复类题目物化后是红树、终态是绿,被拿刚物化的树对 exit 0,资格阶段直接判 `failed`。两处都是靠出题暴露的,不是靠读代码。教训:schema 能表达的组合,必须有一道题实际走通一遍。
